...
GLuint load_png_asset_into_texture(const char* relative_path) {
    ...
    FileData png_file = get_asset_data(relative_path);
...
GLuint build_program_from_assets(
    ...
    FileData vertex_shader_source = get_asset_data(vertex_shader_path);
    FileData fragment_shader_source = get_asset_data(fragment_shader_path);
    GLuint program_object_id = build_program(
